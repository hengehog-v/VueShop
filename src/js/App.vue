<template>
    <div>
        ...
        <section>
            ...
            <Cart v-for="(item, index) in itemsCart" 
            :key="index"
            :item="{...item}"
            @sumChange="sumChange"
            @dellItem="dellItemCart"/>
        </section>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            sumBuy(){
                this.sum = this.itemsCart.reduce((sum, item) => item.productPrice * item.count + sum, 0);
            },

            addItemCart(item) {
                const existedItem = this.itemsCart.filter(itemCart => item.id === itemCart.id)[0];
                if(!existedItem){
                    item.count = 1;
            ...
            sumChange(item, str){
                if(str === '+') {
                    this.itemsCart.forEach(itemCart => {if(item.id === itemCart.id) itemCart.count++})
                    this.sumBuy();
                    }
                else if (str === '-') {
                    if(item.count !== 1){
                        this.itemsCart.forEach(itemCart => {if(item.id === itemCart.id) itemCart.count--
                        this.sumBuy()})
                    } else this.dellItemCart(item);
                }
            },

            dellItemCart(item){
                this.itemsCart = this.itemsCart.filter(itemCart => item.id !== itemCart.id);
                this.sumBuy();
            },
         },
    }
...
<style lang='less' module>
    ...
    * {
    ...
    header {
        background: linear-gradient(90deg, rgb(11,11,11,1) 0%, rgb(85, 85, 85) 42%, rgb(196, 196, 196) 76%);
        display: flex;
